...
use std::fmt::format;
use std::{error::Error, net::SocketAddr, sync::Arc, collections::HashMap, str::FromStr};

use rand::{thread_rng, Rng};
use rand::distributions::Alphanumeric;

use crate::{types::{SenderType, RecieverType, Command}, message::Message};
    ...
    while let Some(command) = reciever.recv().await {
        ...
        match command {
            Command::Response { conn_id, key: thing , data: _} => {
                info!("{}", thing);
                response_message = "hello".as_bytes().to_vec();
                id = conn_id
            }
            _ => {
